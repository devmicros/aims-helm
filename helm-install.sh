...
if [ ! -z "${project}" ]; then
  ...
  if [ -z "${app}" ]; then
  	...
  	oc new-project ${aimsproject} --display-name 'AIMS2'
  ...
  if [ "${project}" == "${aimsproject}" ]; then
    ...
    {
    helm install aims2 aims-chart 2>/dev/null
    } || {
    helm upgrade --install  aims2 aims-chart 2>/dev/null
    } || {
	...
	if [ ! -z "${podname}" ]; then
      ...
      oc get -o yaml  deployment/aims-http > aims2-http.yaml
      oc get -o yaml  deployment/aims-ws > aims2-ws.yaml
    
      sg='s/aims-gql.microsafe.com.mx/'${aimsgql}'/g'
      sed -i ${sg} aims2-http.yaml    
      sed -i ${sg} aims2-ws.yaml
      sg='s/aims-ws.microsafe.com.mx/'${aimsws}'/g'
      sed -i ${sg} aims2-http.yaml    
      sg='s/aims-http.microsafe.com.mx/'${aimshttp}'/g'
      sed -i ${sg} aims2-ws.yaml
           
      oc replace  deployment/aims-http -f aims2-http.yaml
      oc replace  deployment/aims-ws -f aims2-ws.yaml
