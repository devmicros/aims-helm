...
ALTER DATABASE aims SET glob_variables.command_limit_minutes = 5;
ALTER DATABASE aims SET glob_variables.test_steps_limit_minutes = 5;
...
(
    ...
    date TIMESTAMP NOT NULL DEFAULT date_trunc('seconds', CURRENT_TIMESTAMP),

    PRIMARY KEY (system_user_historical_id,date)
);
...
CREATE TABLE IF NOT EXISTS command_type (
    ...
    for_production BOOLEAN NOT NULL DEFAULT TRUE,
...
CREATE TABLE IF NOT EXISTS electronic_lock_tests_steps_events (
    id_electronic_lock_test_step INT NOT NULL,
    id_event INT NOT NULL,

    PRIMARY KEY (id_electronic_lock_test_step,id_event)
);
...
ALTER TABLE IF EXISTS electronic_lock_tests_steps_events
  ADD FOREIGN KEY (id_electronic_lock_test_step)
  REFERENCES electronic_lock_tests_steps (id_electronic_lock_test_step) MATCH SIMPLE
  ON UPDATE NO ACTION
  ON DELETE NO ACTION
  NOT VALID;
...
VALUES
  (3,'Apertura mediante reto (Fascia)',true,'El proceso de apertura randómica del cajero %s fue exitoso.',1),
  ...
  BEGIN
    ...
    RETURN QUERY (
      ...
      FROM events e
        ...
        LEFT JOIN electronic_lock_tests_steps_events eltse ON eltse.id_event = e.id_event
      WHERE CASE WHEN last_event_date IS NOT NULL THEN e.event_date > last_event_date ELSE true END
        AND et.is_notification = true AND eltse.id_electronic_lock_test_step IS NULL
      )
    UNION
      (SELECT 
        a.id_alarm AS id, 
...
$function$
  BEGIN
    ...
    RETURN QUERY
      ...
      FROM command_type ct WHERE ct.for_production = true;
  END;
$function$;
...
$FUNCTION$
  ...
  BEGIN
    ...
    in_process_locks := ARRAY(
      SELECT 
				elt.id_lock
      FROM electronic_lock_tests elt 
      WHERE elt.success IS NULL
      GROUP BY elt.id_lock
    );
    ...
    RETURN QUERY
      SELECT 
        el.id_lock,
        CASE 
          WHEN el.id_lock = ANY(in_process_locks) THEN 'En proceso'
          WHEN el.id_lock = ANY(failure_locks) THEN 'Fallida'
          ELSE 'Por iniciar'
        END as status
      FROM electronic_lock el
        LEFT JOIN electronic_lock_tests elt ON elt.id_lock = el.id_lock
      WHERE el.work_mode = 1 AND NOT(el.id_lock = ANY(success_locks)) GROUP BY el.id_lock;
  END;
$FUNCTION$;
...
$FUNCTION$
  ...
  BEGIN
    ...
    failure_steps_ids := ARRAY(
      SELECT 
			  MAX(elts.id_electronic_lock_test_step)
      FROM electronic_lock_tests elt 
    ...
    RETURN QUERY
      ...
      WHERE elts.id_electronic_lock_test_step = ANY(failure_steps_ids)
        AND
          CASE
            WHEN id_lock_filter IS NOT NULL AND id_lock_filter != '' 
...
$function$
  ...
  BEGIN
    ...
    LOOP
      IF(rec.test_step_date + (test_steps_limit_minutes) * INTERVAL'1 minute' + INTERVAL'5 seconds' < CURRENT_TIMESTAMP) THEN
...
CREATE OR REPLACE FUNCTION get_current_test(
  ...
  OUT step INT,
  OUT test_step_date TIMESTAMPTZ,
  OUT test_steps_limit_time INT,
  OUT random_string TEXT
)
LANGUAGE plpgsql
STABLE
AS 
$FUNCTION$
  ...
  BEGIN
    PERFORM check_token_validity();
    SELECT current_setting('glob_variables.test_steps_limit_minutes', true)::INT*60 INTO test_steps_limit_time;
    SELECT elt.id_electronic_lock_test INTO get_current_test.id_electronic_lock_test
      FROM electronic_lock_tests_steps elts
        INNER JOIN electronic_lock_tests elt ON elt.id_electronic_lock_test = elts.id_electronic_lock_test
        INNER JOIN electronic_lock el ON el.id_lock = elt.id_lock
      WHERE el.work_mode = 1 AND elt.success IS NULL AND el.id_lock = get_current_test.id_lock AND
        NOT (elts.id_electronic_lock_test = ANY(SELECT 
            elt.id_electronic_lock_test
          FROM electronic_lock_tests elt 
            INNER JOIN electronic_lock_tests_steps elts ON 
    ...
    IF(get_current_test.id_electronic_lock_test IS NOT NULL) THEN
      SELECT 
        elts.step, 
        elts.test_step_date::TIMESTAMPTZ
        INTO
      ...
      SELECT c.temp_code INTO random_string FROM commands c
        INNER JOIN electronic_lock el ON el.id_lock = c.id_lock
        INNER JOIN electronic_lock_tests elt ON elt.id_lock = el.id_lock
      WHERE elt.id_electronic_lock_test = get_current_test.id_electronic_lock_test
...
$FUNCTION$
  ...
  BEGIN
    ...
    SELECT MAX(elts.id_electronic_lock_test_step) INTO id_failure_test_step
      FROM electronic_lock_tests_steps elts 
    WHERE elts.id_electronic_lock_test = set_lock_test_failure_details.id_electronic_lock_test
    GROUP BY elts.id_electronic_lock_test;

    UPDATE electronic_lock_tests_steps elts SET
      failed = true
    WHERE elts.id_electronic_lock_test_step = id_failure_test_step;

    RETURN;
  END;
$FUNCTION$;

CREATE OR REPLACE FUNCTION set_lock_test_success(
  IN id_electronic_lock_test INT
)
RETURNS VOID
LANGUAGE plpgsql
...
$FUNCTION$
  ...
  BEGIN
    ...
    UPDATE electronic_lock_tests elt SET
      success = true
    WHERE elt.id_electronic_lock_test = set_lock_test_success.id_electronic_lock_test;
    RETURN;
  END;
$FUNCTION$;

CREATE OR REPLACE FUNCTION get_lock_test_step_events(
  IN id_electronic_lock_test INT,
  IN step INT
)
RETURNS TABLE(
...
$FUNCTION$
  DECLARE 
    test_step_date TIMESTAMP;
    id_lock_test INT;
    current_id_electronic_lock_test_step INT;
  BEGIN
    PERFORM check_token_validity();
    SELECT 
      elts.test_step_date,
      elt.id_lock,
      elts.id_electronic_lock_test_step
    INTO 
      test_step_date,
      id_lock_test,
      current_id_electronic_lock_test_step
    FROM electronic_lock_tests_steps elts
      INNER JOIN electronic_lock_tests elt ON elt.id_electronic_lock_test = elts.id_electronic_lock_test
      WHERE elts.id_electronic_lock_test = get_lock_test_step_events.id_electronic_lock_test
        AND elts.step = get_lock_test_step_events.step;
    
    INSERT INTO electronic_lock_tests_steps_events(
      id_event,
      id_electronic_lock_test_step
    )SELECT 
      e.id_event,
      current_id_electronic_lock_test_step
    FROM events e
    WHERE e.id_lock = id_lock_test AND e.event_date > test_step_date
    ORDER BY e.event_date ON CONFLICT DO NOTHING;

    RETURN QUERY
      SELECT e.id_event_type,e.event_date::TIMESTAMPTZ
      FROM events e
      WHERE e.id_lock = id_lock_test AND e.event_date > test_step_date
...
$FUNCTION$
  ...
  BEGIN
    ...
    LOOP
        ...
        blocked_reason := already_exist_lock.lock_description;
...
GRANT SELECT ON electronic_lock_tests_steps_events TO backend_agent;
...
GRANT SELECT, INSERT ON electronic_lock_tests_steps_events TO system_admin;
...
GRANT SELECT, INSERT ON electronic_lock_tests_steps_events TO test;
GRANT SELECT, UPDATE ON electronic_lock to test;
GRANT SELECT, INSERT, UPDATE ON commands to test;
GRANT SELECT ON events to test;
...
GRANT EXECUTE ON FUNCTION 
  ...
  get_lock_test_step_events,
  set_lock_test_success
To test;
